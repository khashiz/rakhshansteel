...
abstract class RSCommentsHelper
{
	...
	public static function parseComment($comment, $permissions, $display = false) {
		...
		// CODE
		$patterns[] = '#\[code\](.*?)\[\/code\]#ism';
		$replacements[] = (isset($permissions['bb_code']) && $permissions['bb_code']) ? '<span class="rsc_code">'.JText::_('TPL_RSCOMMENTS_CODE').' : <pre>\\1</pre></span>' : '\\1';
		...
		// QUOTE
		$quotePattern = '#\[quote\s?name=\"([^\"\'\<\>\(\)]+)+\"\](<br\s?\/?\>)*(.*?)(<br\s?\/?\>)*\[\/quote\]#i';
		$quoteReplace = '<span class="rsc_quote"> <strong>\\1</strong> '.JText::_('TPL_RSCOMMENTS_USER_SAID').': <blockquote>\\3</blockquote> </span>';
		while(preg_match($quotePattern, $comment)) {
			$comment = preg_replace($quotePattern, $quoteReplace, $comment);
		}
		$quotePattern = '#\[quote[^\]]*?\](<br\s?\/?\>)*([^\[]+)(<br\s?\/?\>)*\[\/quote\]#i';
		$quoteReplace = '<span class="rsc_quote">'.JText::_('TPL_RSCOMMENTS_QUOTE_SINGLE').' : <blockquote>\\2</blockquote> </span>';
		while(preg_match($quotePattern, $comment)) {
			$comment = preg_replace($quotePattern, $quoteReplace, $comment);
		}
	...
	public static function showIcons($permissions) {
		...
		if ($config->enable_smiles == 1) {
			$smileys = array('<a href="javascript:void(0);" data-rsc-task="emoticons" class="btn btn-small '.RSTooltip::tooltipClass().'" title="'.RSTooltip::tooltipText(JText::_('TPL_RSCOMMENTS_EMOTICONS')).'"><i class="fa fa-smile-o"></i></a>');
	...
	public static function createBBs($permissions) {
		...
		if (isset($permissions['bb_bold']) && $permissions['bb_bold']) {
			$bbcode[] = '<a href="javascript:void(0);" data-rsc-task="bbcode" data-rsc-code="bold" class="btn btn-small '.RSTooltip::tooltipClass().'" title="'.RSTooltip::tooltipText(JText::_('TPL_RSCOMMENTS_BOLD')).'"><i class="fa fa-bold"></i></a>';
		}
		
		if (isset($permissions['bb_italic']) && $permissions['bb_italic']) {
			$bbcode[] = '<a href="javascript:void(0);" data-rsc-task="bbcode" data-rsc-code="italic" class="btn btn-small '.RSTooltip::tooltipClass().'" title="'.RSTooltip::tooltipText(JText::_('TPL_RSCOMMENTS_ITALIC')).'"><i class="fa fa-italic"></i></a>';
		}
		
		if (isset($permissions['bb_underline']) && $permissions['bb_underline']) {
			$bbcode[] = '<a href="javascript:void(0);" data-rsc-task="bbcode" data-rsc-code="underline" class="btn btn-small '.RSTooltip::tooltipClass().'" title="'.RSTooltip::tooltipText(JText::_('TPL_RSCOMMENTS_UNDERLINE')).'"><i class="fa fa-underline"></i></a>';
		}
		
		if (isset($permissions['bb_stroke']) && $permissions['bb_stroke']) {
			$bbcode[] = '<a href="javascript:void(0);" data-rsc-task="bbcode" data-rsc-code="stroke" class="btn btn-small '.RSTooltip::tooltipClass().'" title="'.RSTooltip::tooltipText(JText::_('TPL_RSCOMMENTS_STROKE')).'"><i class="fa fa-strikethrough"></i></a>';
		}
		
		if (isset($permissions['bb_quote']) && $permissions['bb_quote']) {
			$bbcode[] = '<a href="javascript:void(0);" data-rsc-task="bbcode" data-rsc-code="quote" class="btn btn-small '.RSTooltip::tooltipClass().'" title="'.RSTooltip::tooltipText(JText::_('TPL_RSCOMMENTS_QUOTE')).'"><i class="fa fa-quote-right"></i></a>';
		}
		
		if (isset($permissions['bb_lists']) && $permissions['bb_lists']) {
			$bbcode[] = '<a href="javascript:void(0);" data-rsc-task="bbcode" data-rsc-code="list" class="btn btn-small '.RSTooltip::tooltipClass().'" title="'.RSTooltip::tooltipText(JText::_('TPL_RSCOMMENTS_UNORDERED')).'"><i class="fa fa-list-ul"></i></a>';
			$bbcode[] = '<a href="javascript:void(0);" data-rsc-task="bbcode" data-rsc-code="olist" class="btn btn-small '.RSTooltip::tooltipClass().'" title="'.RSTooltip::tooltipText(JText::_('TPL_RSCOMMENTS_ORDERED')).'"><i class="fa fa-list-ol"></i></a>';
		}
		
		if (isset($permissions['bb_image']) && $permissions['bb_image']) {
			$bbcode[] = '<a href="javascript:void(0);" data-rsc-task="bbcode" data-rsc-code="image" data-rsc-text="'.htmlentities(JText::_('TPL_RSCOMMENTS_ADD_IMAGE'), ENT_COMPAT, 'UTF-8').'" class="btn btn-small '.RSTooltip::tooltipClass().'" title="'.RSTooltip::tooltipText(JText::_('TPL_RSCOMMENTS_ADD_IMAGE')).'"><i class="fa fa-picture-o"></i></a>';
		}
		
		if (isset($permissions['bb_url']) && $permissions['bb_url']) {
			$bbcode[] = '<a href="javascript:void(0);" data-rsc-task="bbcode" data-rsc-code="url" data-rsc-text="'.htmlentities(JText::_('TPL_RSCOMMENTS_ADD_URL'), ENT_COMPAT, 'UTF-8').'" class="btn btn-small '.RSTooltip::tooltipClass().'" title="'.RSTooltip::tooltipText(JText::_('TPL_RSCOMMENTS_ADD_URL')).'"><i class="fa fa-link"></i></a>';
		}
		
		if (isset($permissions['bb_code']) && $permissions['bb_code']) {
			$bbcode[] = '<a href="javascript:void(0);" data-rsc-task="bbcode" data-rsc-code="code" class="btn btn-small '.RSTooltip::tooltipClass().'" title="'.RSTooltip::tooltipText(JText::_('TPL_RSCOMMENTS_ADD_CODE')).'"><i class="fa fa-code"></i></a>';
		}
		
		if (isset($permissions['bb_videos']) && $permissions['bb_videos']) {
			$bbcode[] = '<a href="javascript:void(0);" data-rsc-task="bbcode" data-rsc-code="youtube" class="btn btn-small '.RSTooltip::tooltipClass().'" title="'.RSTooltip::tooltipText(JText::_('TPL_RSCOMMENTS_ADD_YOUTUBE')).'"><i class="fa fa-youtube"></i></a>';
			$bbcode[] = '<a href="javascript:void(0);" data-rsc-task="bbcode" data-rsc-code="vimeo" class="btn btn-small '.RSTooltip::tooltipClass().'" title="'.RSTooltip::tooltipText(JText::_('TPL_RSCOMMENTS_ADD_VIMEO')).'"><i class="fa fa-vimeo"></i></a>';
		}
	...
	public static function name($comment, $permissions) {
		...
		if ($comment->uid == 0) {
			...
			if ($comment->anonymous) {
				$cleanname = empty($cleanname) ? JText::_('TPL_RSCOMMENTS_ANONYMOUS') : $cleanname;
			}
	...
	public static function displayForm($option, $id, $override = false) {
		...
		if ($config->form_accordion) {
			JText::script('TPL_RSCOMMENTS_HIDE_FORM');
			JText::script('TPL_RSCOMMENTS_SHOW_FORM');
		}
...
");
	...
	public static function humanReadableDate($date) {
		...
		if ($diff_secs == 0) {
			return JText::_('TPL_RSCOMMENTS_JUST_NOW');
		}
		...
		foreach ($units as $unit) {
			if ( ($data->$unit || $current_precision) && $current_precision < $precision ) {
				if ($data->$unit) {
					$result[] = JText::plural( 'TPL_RSCOMMENTS_NUMBER_OF_' . strtoupper($unit), $data->$unit );
				}
