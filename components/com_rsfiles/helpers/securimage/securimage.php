...
class JSecurImage {
	...
	*/
	...
	function __construct()
	{
		...
		$this->wordlist_file = JPATH_SITE.'/components/com_rsfiles/helpers/securimage/words/words.txt';
		$this->use_wordlist  = false;

		$this->gd_font_file  = JPATH_SITE.'/components/com_rsfiles/helpers/securimage/gdfonts/automatic.gdf';
		$this->use_gd_font   = false;
		$this->gd_font_size  = 24;
		$this->text_x_start  = 15;

		$this->ttf_file      = JPATH_SITE.'/components/com_rsfiles/helpers/securimage/AHGBold.ttf';
		...
		$this->signature_font  = JPATH_SITE.'/components/com_rsfiles/helpers/securimage/AHGBold.ttf';

		$this->audio_path   = JPATH_SITE.'/components/com_rsfiles/helpers/securimage/audio/';
	...
	function saveData()
	{
		...
		$session->set('com_rsfiles.securimage.value', $this->code);
		$session->set('com_rsfiles.securimage.ctime', time());
	}
	...
	function validate()
	{
		...
		if (!RSFilesHelper::getConfig('captcha_case_sensitive'))
		{
		...
		if ($code != '' && $code == $code_entered)
		{
			...
			$session->set('com_rsfiles.securimage.value', '');
			$session->set('com_rsfiles.securimage.ctime', '');
		}
	}
	...
	{
		$session = JFactory::getSession();
		$creation_time = $session->get('com_rsfiles.securimage.ctime', 0);
		$expired = true;
