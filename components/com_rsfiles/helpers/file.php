...
class RSFiles {
	...
	public function __construct($fullpath, $options = null) {
		$this->admin			= JFactory::getApplication()->isClient('administrator');
	...
	protected function getInfo() {
		...
		$object->name				= $type == 'external' ? (!empty($entry->DownloadName) ? $entry->DownloadName : rsfilesHelper::getName($entry->FilePath)) : rsfilesHelper::getName($this->fullpath);
		$object->type				= $type;
		...
		$object->description		= isset($entry->FileDescription) ? $entry->FileDescription : '';
		...
		if ($type == 'file' || $type == 'external') {
			...
			$object->size				= !empty($entry->FileSize) ? $entry->FileSize : ($type == 'external' ? '-' : rsfilesHelper::formatBytes(rsfilesHelper::filesize($this->fullpath)));
			...
			$object->poster				= isset($entry->poster) ? $entry->poster : '';
			$ext						= isset($entry->icon) && !empty($entry->icon) ? $entry->icon : strtolower(rsfilesHelper::getExt($this->fullpath));
			$object->icon				= in_array($ext, rsfilesHelper::fileExtensions()) ? 'flaticon-'.$ext.'-file' : 'fa fa-file';
			
			if (in_array($object->extension,$audio)) {
				$object->previewtype = 'mp3';
			} elseif (in_array($object->extension,$video)) {
				$object->previewtype = 'm4v';
			} else $object->previewtype = '';
			
			if (JFactory::getApplication()->isClient('site')) {
				$object->filelicense 	= empty($entry->IdLicense) ? '' : '<a href="javascript:void(0);" onclick="rsfiles_show_modal(\''.JRoute::_('index.php?option=com_rsfiles&layout=license&tmpl=component&id='.rsfilesHelper::sef($entry->IdLicense,$entry->LicenseName).$itemid,false).'\',\''.$entry->LicenseName.'\',\'800\'">'.$entry->LicenseName.'</a>';
			} else {
		...
		$paths = explode($ds, $fullpath);
		array_pop($paths);
		
		$object->path = JText::_('COM_RSFILES_ROOT') . (!empty($paths) ? $ds . implode($ds,$paths) : '');
