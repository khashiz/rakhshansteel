...
class rsfilesHelper {
	...
	// Load Config
	public static function getConfig($what = null, $default = false) {
		static $config;
	...
	public static function initialize($from = 'admin') {
		...
		JHtml::stylesheet('com_rsfiles/flaticon.css', array('relative' => true, 'version' => 'auto'));
		
		if ($from == 'admin') {
			JHtml::_('formbehavior.chosen', 'select');
			
			JHtml::script('com_rsfiles/admin.js', array('relative' => true, 'version' => 'auto'));
			JHtml::stylesheet('com_rsfiles/admin.css', array('relative' => true, 'version' => 'auto'));
		} else {
			...
			JHtml::script('com_rsfiles/site.js', array('relative' => true, 'version' => 'auto'));
			JHtml::stylesheet('com_rsfiles/site.css', array('relative' => true, 'version' => 'auto'));
		}
	}
	...
	public static function loadFA() {
		$admin	= JFactory::getApplication()->isClient('administrator');
		$config = self::getConfig();
		$enable = $admin ? $config->load_fa : $config->load_frontend_fa;
		
		if ($enable) {
			JHtml::stylesheet('com_rsfiles/fontawesome-all.min.css', array('relative' => true, 'version' => 'auto'));
		}
	}
	...
	public static function subMenu() {
		...
		$views  = array('files','tags','licenses','groups','statistics','rules','settings');
	...
	public static function modalClose($script = true, $parent = false) {
		$html	= '';
		$modal	= rsfilesHelper::getConfig('modal');
		$modal	= $modal == 1 ? "jQuery('#rsfRsfilesModal').modal('hide');" : 'jQuery.magnificPopup.close();';
		
		if ($script) $html .= '<script type="text/javascript">'."\n";
		if ($parent) $html .= 'window.parent.';
		$html .= $modal;
	...
	public static function upload($id) {
		...
		$poster		= $input->files->get('poster',array(),'array');
		...
		if (!empty($poster)) {
			$extension = rsfilesHelper::getExt($poster['name']);
			
			if (in_array(strtolower($extension),array('png','jpg','jpeg'))) {
				$dest	= JPATH_SITE.'/components/com_rsfiles/images/poster/';
				if (!$poster['error'] && !empty($poster['tmp_name'])) {
					$filename	= basename(JFile::stripExt($poster['name']));
					
					while(JFile::exists($dest.$filename.'.'.$extension))
						$filename .= rand(1,999);
					
					if (JFile::upload($poster['tmp_name'], $dest.$filename.'.'.$extension)) {
						$query->clear()
							->update($db->qn('#__rsfiles_files'))
							->set($db->qn('poster').' = '.$db->q($filename.'.'.$extension))
							->where($db->qn('IdFile').' = '.(int) $id);
						$db->setQuery($query);
						$db->execute();
					}
				...
				if ($poster['error'] == 0 && $poster['size'] > 0) {
					$app->enqueueMessage(JText::_('COM_RSFILES_INVALID_POSTER_EXTENSION'),'error');
				}
			}
		}
			...
			foreach ($screens as $screen) {
				...
				if (in_array(strtolower($extension),array('png','jpg','gif'))) {
					if (!$screen['error'] && !empty($screen['tmp_name'])) {
						$filename	= basename(JFile::stripExt($screen['name']));
	...
	public static function getIP($check_for_proxy = false) {
		if (!rsfilesHelper::getConfig('store_ip')) {
			return '0.0.0.0';
		}
	...
	public static function statistics($aPath, $rPath, $external = false) {
		...
		$referer	= $email && empty($referer) ? 'email' : $referer;
		...
		if ($file = $db->loadObject()) {
			...
			$query->clear()
				->insert($db->qn('#__rsfiles_files'))
				->set($db->qn('FilePath').' = '.$db->q($rPath))
				->set($db->qn('DateAdded').' = '.$db->q(JFactory::getDate(filemtime($aPath))->toSql()))
				->set($db->qn('ModifiedDate').' = '.$db->q(JFactory::getDate()->toSql()))
				->set($db->qn('FileDescription').' = '.$db->q(''))
				->set($db->qn('metadescription').' = '.$db->q(''))
				->set($db->qn('metakeywords').' = '.$db->q(''))
	 ...
	 *
	 */
	public static function briefcase($permission, $userID = null) {
		$db 			= JFactory::getDbo();
		$query			= $db->getQuery(true);
	...
	public static function previewExtensions($tostring = false) {
		$supported = array('jpg','jpeg','txt','png','pdf','gif','mp3','mp4','mov','m4v');
	...
	public static function filepath() {
		...
		$fullpath	= $isExternal ? $fullpath : realpath($fullpath);
	...
	public static function getOrdering() {
		...
		$options[] = JHtml::_('select.option','name',JText::_('COM_RSFILES_SEARCH_NAME'));
		$options[] = JHtml::_('select.option','date',JText::_('COM_RSFILES_SEARCH_DATE'));
		
		if (!rsfilesHelper::isBriefcase()) {
			$options[] = JHtml::_('select.option','hits',JText::_('COM_RSFILES_SEARCH_HITS'));
		}
	...
	public static function downloadlink($file, $path) {
		...
		$download->ismodal 		= $ismodal;
		$download->size 		= $size;
		$download->enablemodal 	= '';
		$download->rel 			= '';
	...
	public static function display($path, $fparams) {
		...
		// This will override the path option
		if ($tags = $fparams->get('tags', null)) {
			$type = 'tags';
		}
		...
		if ($type == 'folder') {
			...
			if ($tags = $fparams->get('tags', null)) {
				$query->clear()
					...
					->where($db->qn('title').' IN ('.rsfilesHelper::quoteImplode($tags).')');
				$db->setQuery($query);
				if ($tags = $db->loadColumn()) {
					$tags = array_map('intval',$tags);
					
					$query->clear()
						->select('DISTINCT '.$db->qn('f.FilePath'))
					...
					if ($files = $db->loadColumn()) {
						...
						foreach ($files as $file) {
							if (is_file(realpath($download_folder.'/'.$file))) {
								$array[] = realpath($download_folder.'/'.$file);
							}
						}
						
						$class		= new RSFilesFiles($array, 'tags', $itemid, 0, $fparams->get('ordering', 'name'), $fparams->get('order', 'ASC'));
						$files		= $class->getFiles();
						$external	= $class->getExternal(array('tags' => $tags));
						
						$view->items = array_merge($files,$external);
					}
				}
			}
		}
	...
	public static function getNavigation() {
		...
		if ($elements = explode($ds, $folder)) {
			foreach ($elements as $i => $element) {
				...
				$query->clear()->select($db->qn('FileName'))->from($db->qn('#__rsfiles_files'))->where($db->qn('FilePath').' = '.$db->q(urldecode($navigation)));
				$db->setQuery($query);
				$name = $db->loadResult();
				
				$newelement->name = $name ? $name : $element;
	...
	public static function preview($id) {
		...
		if (!empty($id)) {
			...
			if ($file = $db->loadObject()) {
				if (!empty($file->preview)) {
					...
					if ($file->FileType) $external = $file->FilePath;
		...
		if (!in_array($extension,$allowed)) {
			...
			if ($extension == 'txt') {
				...
				if ($config->modal == 2) {
					echo '<style type="text/css">pre{white-space:pre-wrap;background: #fff;}</style>';
				}
			...
			if (in_array($extension,$audio)) {
				$doc->addCustomTag('<script src="'.JHtml::script('com_rsfiles/jplayer.min.js', array('relative' => true, 'pathOnly' => true, 'version' => 'auto')).'" type="text/javascript"></script>');
				JHtml::stylesheet('com_rsfiles/jplayer.min.css', array('relative' => true, 'version' => 'auto'));
				
				$js	  = array();
				$js[] = 'jQuery(document).ready(function(){';
				$js[] = "\t".'jQuery("#rsfiles-audio-player-'.$hash.'").jPlayer({';
				...
				$js[] = "\t\t".'swfPath: "'.JURI::root().'media/com_rsfiles/images/",';
				$js[] = "\t\t".'wmode: "window",';
				...
				$js[] = "\t\t".'cssSelectorAncestor: "#jp_container-'.$hash.'",';
				...
				echo '<div class="rsfiles-audio-player-container'.($config->modal == 2 ? '-mpModal' : '').'">'."\n";
				echo '<div id="rsfiles-audio-player-'.$hash.'" class="jp-jplayer"></div>'."\n";
				echo "\t".'<div id="jp_container-'.$hash.'" class="jp-audio" role="application" aria-label="media player">'."\n";
				echo "\t\t".'<div class="jp-type-single">'."\n";
				echo "\t\t\t".'<div class="jp-gui jp-interface">'."\n";
				echo "\t\t\t\t".'<div class="jp-controls">'."\n";
				echo "\t\t\t\t\t".'<button class="jp-play" role="button" >&nbsp;</button>'."\n";
				echo "\t\t\t\t\t".'<button class="jp-stop" role="button" >&nbsp;</button>'."\n";
				echo "\t\t\t\t".'</div>'."\n";
				...
				echo "\t\t\t\t".'<button class="jp-mute" role="button" ">&nbsp;</button>'."\n";
				echo "\t\t\t\t".'<button class="jp-volume-max" role="button" ">&nbsp;</button>'."\n";
			...
			if (in_array($extension,$video)) {
				...
				$js	  = array();
				$js[] = 'jQuery(document).ready(function(){';
				$js[] = "\t".'jQuery("#rsfiles-video-player-'.$hash.'").jPlayer({';
				$js[] = "\t\t".'ready: function (event) {';
				$js[] = "\t\t\t".'jQuery(this).jPlayer("setMedia", {';
				$js[] = "\t\t\t\t".'m4v: "'.$thepath.'"';
				$js[] = "\t\t\t".'});';
				...
				echo '<div class="rsfiles-video-player-container'.($config->modal == 2 ? '-mpModal' : '').'">'."\n";
				echo '<div id="jp_container_1" class="jp-video jp-video-360p" role="application" aria-label="media player">'."\n";
				echo "\t".'<div class="jp-type-single">'."\n";
				echo "\t".'<div id="rsfiles-video-player-'.$hash.'" class="jp-jplayer"></div>'."\n";
				echo "\t".'<div class="jp-gui">'."\n";
				echo "\t".'<div class="jp-video-play">'."\n";
				echo "\t".'<button class="jp-video-play-icon" role="button">play</button>'."\n";
				echo "\t".'</div>'."\n";
				...
				echo "\t".'<button class="jp-full-screen" role="button">&nbsp;</button>'."\n";
				echo "\t".'</div>'."\n";
				echo "\t".'</div>'."\n";
				echo "\t".'</div>'."\n";
				echo "\t".'</div>'."\n";
	...
	public static function previewProperties($id, $path = null) {
		...
		if ($extension == 'mp3' || $extension == 'ogg') {
			$size = array('width' => 650, 'height' => 100);
		} else if (in_array($extension,array('mp4','mov','webm','m4v'))) {
			$size = array('width' => 700, 'height' => 450);
		} else {
			$size = array('width' => 660, 'height' => 475);
		}
	...
	public static function externalHeaders($url, $type, $default = '') {
		...
		return isset($headers[$type]) ? $headers[$type] : $default;
	}
	
	public static function getExternalName($url) {
		$headers = rsfilesHelper::externalHeaders($url, true);
		
		// Search for the file name in the Content-Disposition header
		if (isset($headers['content-disposition'])) {
			if (preg_match('#filename=\"(.*?)\"#is',$headers['content-disposition'],$match)) {
				if (isset($match[1])) {
					return $match[1];
				}
			}
		...
		// Search for the file name in the Location header
		if (isset($headers['Location'])) {
			jimport('joomla.filesystem.file');
			
			$name	= basename($headers['Location']);
	...
	public static function externalStatus($url) {
		$headers	= rsfilesHelper::externalHeaders($url, true);
	...
	public static function fileExtensions() {
		$extensions = array('3dm', '3ds', '3g2', '3gp', '7z', 'aac', 'ai', 'aif', 'apk', 'app', 'asf', 'asp', 'aspx', 'asx', 'avi', 'bak', 'bat', 'bin', 'bmp', 'cab', 'cad', 'cdr', 'cer', 'cfg', 'cfm', 'cgi', 'class', 'com', 'cpl', 'cpp', 'crx', 'csr', 'css', 'csv', 'cue', 'cur', 'dat', 'db', 'dbf', 'dds', 'dem', 'dfx', 'dll', 'dmg', 'dmp', 'doc', 'docx', 'drv', 'dtd', 'dwg', 'elf', 'eps', 'eps1', 'exe', 'fla', 'flash', 'flv', 'fnt', 'fon', 'gam', 'gbr', 'ged', 'gif', 'gpx', 'gz', 'gzip', 'hqz', 'html', 'ibooks', 'icns', 'ico', 'ics', 'iff', 'indd', 'iso', 'iso', 'jar', 'jpg', 'js', 'jsp', 'key', 'kml', 'kmz', 'lnk', 'log', 'lua', 'm3u', 'm4a', 'm4v',  'max', 'mdb', 'mdf', 'mid', 'mim', 'mov', 'mp3', 'mp4', 'mpa', 'mpg', 'msg', 'msi', 'nes', 'obj', 'odb', 'odc', 'odf', 'odg', 'odi', 'odp', 'ods', 'odt', 'odt1', 'odx', 'ogg', 'otf', 'otf1', 'pages', 'pct', 'pdb', 'pdf', 'photoshop', 'pif', 'pkg', 'pl', 'png', 'pps', 'ppt', 'pptx', 'ps', 'pub', 'py', 'ra', 'rar', 'raw', 'rm', 'rom', 'rpm', 'rss', 'rtf', 'sav', 'sdf', 'sitx', 'sql', 'sql', 'srt', 'svg', 'swf', 'sys', 'tar', 'tex', 'tga', 'thm', 'tiff', 'tmp', 'torrent', 'ttf', 'txt', 'uue', 'vb', 'vcd', 'vcf', 'vob', 'wav', 'wma', 'wmv', 'wpd', 'wps', 'wsf', 'xhtml', 'xlr', 'xls', 'xlsx', 'xml', 'yuv', 'zip');
		
		return $extensions;
	}
	
	public static function tags($id, $from = 'jform') {
		$db		= JFactory::getDbo();
		$query	= $db->getQuery(true);
		$jform	= JFactory::getApplication()->input->get($from, array(), 'array');
		$tags	= isset($jform['tags']) ? $jform['tags'] : array();
		...
		$query->clear()
			->delete()
			->from($db->qn('#__rsfiles_tag_relation'))
			->where($db->qn('file').' = '.(int) $id);
		...
		if (!empty($tags)) {
			foreach ($tags as $tag) {
				$query->clear()
					->insert($db->qn('#__rsfiles_tag_relation'))
					->set($db->qn('file').' = '.(int) $id)
					->set($db->qn('tag').' = '.(int) $tag);
	...
	public static function isTagMenu() {
		$menu	= JFactory::getApplication()->getMenu();
		
		if ($active = $menu->getActive()) {
			if (isset($active->query)) {
				if (isset($active->query['option']) && isset($active->query['layout'])) {
					if ($active->query['option'] == 'com_rsfiles' && $active->query['layout'] == 'tags') {
						return true;
					}
				}
			}
	...
	public static function getBookmarks() {
		...
		return array('download_folder' => $download_folder, 'briefcase_folder' => $briefcase_folder);
	}
	
	public static function getUserBookmarks() {
		$data	= array('download_folder' => array(), 'briefcase_folder' => array());
		
		if ($id = JFactory::getUser()->get('id')) {
			$db		= JFactory::getDbo();
			$query	= $db->getQuery(true);
			$config	= rsfilesHelper::getConfig();
			
			$d_folder = realpath($config->download_folder);
			$b_folder = realpath($config->briefcase_folder);
			
			$query->clear()
				->select($db->qn('file'))
				->from($db->qn('#__rsfiles_bookmarks'))
				->where($db->qn('uid').' = '.$db->q($id));
			$db->setQuery($query);
			if ($bookmarks = $db->loadColumn()) {
				foreach ($bookmarks as $bookmark) {
					$bookmark = realpath($bookmark);
					
					if (strpos($bookmark, $d_folder) !== false) {
						$data['download_folder'][] = $bookmark;
					} elseif (strpos($bookmark, $b_folder) !== false) {
						$data['briefcase_folder'][] = $bookmark;
					}
				}
			}
		}
	...
	public static function addUserBookmark($file) {
		if ($id = JFactory::getUser()->get('id')) {
			$db		= JFactory::getDbo();
			$query	= $db->getQuery(true);
			...
			$db->setQuery($query);
			if (!$bid = (int) $db->loadResult()) {
				$query->clear()
					->insert($db->qn('#__rsfiles_bookmarks'))
					->set($db->qn('uid').' = '.$db->q($id))
					->set($db->qn('file').' = '.$db->q($file));
				$db->setQuery($query);
				$db->execute();
			}
		}
	...
	public static function removeUserBookmark($file) {
		...
		if ($id && $file) {
			...
			$query->clear()
				->delete($db->qn('#__rsfiles_bookmarks'))
				->where($db->qn('uid').' = '.$db->q($id))
				->where($db->qn('file').' = '.$db->q($file));
	...
	public static function isBookmarked($path) {
		$session	= JFactory::getSession();
		...
		if ($briefcase) {
			$file = $bfolder.$ds.$path;
			if (isset($data['briefcase_folder']) && array_search($file, $data['briefcase_folder']) !== false) {
				return true;
			}
		} else {
			$file = $dfolder.$ds.$path;
			if (isset($data['download_folder']) && array_search($file, $data['download_folder']) !== false) {
				return true;
			}
		}
	...
	public static function deleteBookmark($path) {
		$ds			= rsfilesHelper::ds();
		$uid		= JFactory::getUser()->get('id');
		$config		= rsfilesHelper::getConfig();
		$dbfolder	= rsfilesHelper::briefcase('CanMaintainBriefcase') ? $config->briefcase_folder : $config->briefcase_folder.$ds.$uid;
		$session	= JFactory::getSession();
		$dfolder	= $session->get('rsfilesdownloadfolder');
		$bfolder	= $session->get('rsfilesbriefcasefolder', $dbfolder);
		$default	= array('download_folder' => array(), 'briefcase_folder' => array());
		$bookmarks	= $session->get('rsfl_bookmarks', $default);
		$path	 	= urldecode($path);
		$d_file		= realpath($dfolder.$ds.$path);
		$b_file		= realpath($bfolder.$ds.$path);
		
		rsfilesHelper::removeUserBookmark($d_file);
		rsfilesHelper::removeUserBookmark($b_file);
		...
		if (!empty($bookmarks['download_folder'])) {
			foreach($bookmarks['download_folder'] as $j => $bookmark) {
				if ($bookmark == $d_file) {
					unset($bookmarks['download_folder'][$j]);
				}
			}
		}
