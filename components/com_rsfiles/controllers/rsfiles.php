...
class rsfilesControllerRsfiles extends JControllerLegacy
{
	...
	public function report() {
		...
		echo rsfilesHelper::modalClose(false, true);
		echo "</script>";
		exit();
	}
	...
	public function bookmark() {
		...
		rsfilesHelper::showResponse($data->status, $data);
	}
	...
	// Method to remove bookmarks
	public function deletebookmarks() {
		// Get the model
		$model		= $this->getModel('rsfiles');
		$pks		= JFactory::getApplication()->input->get('cid',array(),'array');
		
		$model->deletebookmarks($pks);
		$this->setMessage(JText::_('COM_RSFILES_BOOKMARKS_REMOVED'));
		$this->setRedirect(JRoute::_('index.php?option=com_rsfiles&layout=bookmarks',false));
	}
	...
	public function delete() {
		...
		$return = rsfilesHelper::isTagMenu() ? JRoute::_('index.php?option=com_rsfiles&layout=tags') : JRoute::_('index.php?option=com_rsfiles'.($briefcase ? '&layout=briefcase' : '').($path ? '&folder='.urlencode($path) : ''));
	...
	public function emaildownload() {
		...
		if (!$model->emaildownload()) {
			...
			echo rsfilesHelper::modalClose(true, true);
			die();
		}
	}
