...
class rsfilesModelSettings extends JModelAdmin
{
	...
	public function save($data) {
		...
		$dsecure = $input->getInt('rsfl_htaccess',0);
		$bsecure = $input->getInt('rsfl_htaccess_briefcase',0);
		...
		if ($overwrite = $input->getInt('overwrite_briefcase_name',0)) {
			jimport('joomla.filesystem.folder');
			
			if ($folders = JFolder::folders($config->briefcase_folder)) {
				$folders = array_map('intval',$folders);
				$type = $data['briefcase_name'] == 1 ? 'name' : 'username';
				
				foreach ($folders as $folder) {
					$briefcase_name = JFactory::getUser($folder)->get($type);
					
					$query->clear()
						->update($db->qn('#__rsfiles_files'))
						->set($db->qn('FileName').' = '.$db->q(JText::sprintf('COM_RSFILES_BRIEFCASE_FOLDER_LABEL', $briefcase_name)))
						->where($db->qn('briefcase').' = '.$db->q(1))
