...
class rsfilesModelFiles extends JModelList
{
	...
	public function __construct($config = array()) {
		...
		if (empty($root)) {
			$this->_app->enqueueMessage(JText::sprintf('COM_RSFILES_NO_FOLDER_DEFINED',ucwords(str_replace('_',' ',rsfilesHelper::getRoot().'_folder'))), 'error');
			$this->_app->redirect('index.php?option=com_rsfiles&view=settings');
		}
		...
		if (!empty($folder) && strpos(realpath($this->_folder), realpath($root)) !== 0) {
			$this->_app->enqueueMessage(JText::_('COM_RSFILES_OUTSIDE_OF_ROOT'), 'error');
			$this->_app->redirect('index.php?option=com_rsfiles&view=files');
		}
		
		// Compute hash
		$this->hash = md5($this->_folder);
		
		$this->context = 'com_rsfiles.files_'.$this->hash;
		
		parent::__construct($config);
		
		$this->_app->input->set('filter_search', $this->getState('filter.search'));
	}
	...
	public function getPagination() {
		...
		$this->_pagination = new JPagination($this->getTotalData(), $this->getState('list.start'), $this->getState('list.limit'));
	...
	public function getData() {
		...
		$orderdir	= $this->getDirection();
		$external	= array();

		require_once JPATH_SITE.'/components/com_rsfiles/helpers/files.php';
		
		$theclass	= new RSFilesFiles($this->_folder, 'admin', '', 0, $order, $orderdir);
		...
		if ($this->getState('list.limit') != 0)
			$container = array_slice($container,$this->getState('list.start'),$this->getState('list.limit'));
	...
	public function create($folder) {
		...
		if (strlen($dir) > 1) {
			...
			if (JFolder::create($fullpath)) {
				...
				$table->briefcase = (int) $briefcase;
				$table->store();
				$this->_app->setUserState('list.limitstart', 0);
	...
	public function checkupload() {
		...
		$thefile 		= rsfilesHelper::makeSafe(basename($file));
		if ($iOS) {
	...
	public function upload() {
		...
		if ($finished) {
			...
			$this->_app->setUserState($this->context.'.limitstart', 0);
	...
	public function briefcase($id) {
		...
		if (JFolder::create($path)) {
			...
			$table->IdFile = null;
			$table->FilePath = $id;
			$table->FileName = JText::sprintf('COM_RSFILES_BRIECASE_FOLDER_LABEL', $name);
			$table->DateAdded = JFactory::getDate()->toSql();
			$table->briefcase = 1;
			$table->published = 1;
			$table->store();
			...
			foreach ($pks as $pk) {
				...
				if (!empty($id)) {
					...
					if (!is_dir($fullpath)) {
						...
						$query->set($db->qn('DownloadLimit').' = '.$db->q((int) $batch['DownloadLimit']));
					}
					...
					if (!is_dir($fullpath)) {
						$query->set($db->qn('DateAdded').' = '.$db->q(JFactory::getDate(filemtime(realpath($fullpath)))->toSql()));
				...
				// Set tags
				if (isset($batch['tags']) && is_array($batch['tags']) && $canHaveTags) {
					rsfilesHelper::tags($id, 'batch');
				}
			}
	...
	public function getFilterForm($data = array(), $loadData = true) {
		...
		$form->setValue('root','list', rsfilesHelper::getRoot());
