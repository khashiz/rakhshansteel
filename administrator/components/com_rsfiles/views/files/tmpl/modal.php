				...
				<tbody>
					...
					<tr class="row<?php echo $i % 2; ?>">
						...
						<td>
							...
							<a href="javascript:void(0)" onclick="window.parent.<?php echo $function; ?>(<?php if ($this->from == 'editor') echo "'".$this->editor."',"; ?>'<?php echo addslashes($path); ?>');">
								<?php echo $item->name; ?>
							</a> 
							<span class="rsfl_options">
								<a class="btn btn-mini" href="javascript:void(0);" onclick="window.parent.<?php echo $function; ?>(<?php if ($this->from == 'editor') echo "'".$this->editor."',"; ?>'<?php echo addslashes($path); ?>');"><?php echo JText::_('COM_RSFILES_SELECT');?></a>
								<a class="btn btn-mini" href="<?php echo JRoute::_('index.php?option=com_rsfiles&view=files&layout=modal'.($this->from == 'editor' ? '&from=editor' : '').'&tmpl=component&folder='.$item->fullpath.($this->editor ? '&editor='.$this->editor : '')); ?>"><?php echo JText::_('COM_RSFILES_OPEN'); ?></a>
							</span>
						</td>
						<?php } ?>
