...
<div class="row-fluid">
	<div class="span12">
		...
		<form action="<?php echo htmlentities(JURI::getInstance(), ENT_COMPAT, 'UTF-8'); ?>" method="post" id="adminForm" name="adminForm">
			...
			<div class="rsfiles-tags-container well well-small">
				<a href="javascript:void(0)" onclick="document.getElementById('filter_tag').value = '0';document.adminForm.submit();" class="btn btn-small btn-<?php echo $this->tag == 0 ? 'success' : 'primary'; ?>"><?php echo JText::_('COM_RSFILES_ALL'); ?></a>
				<?php foreach ($this->tags as $tag) { ?>
				<a href="javascript:void(0)" onclick="document.getElementById('filter_tag').value = '<?php echo $tag->id; ?>';document.adminForm.submit();" class="btn btn-small btn-<?php echo $this->tag == $tag->id ? 'success' : 'primary'; ?>"><?php echo $tag->title; ?></a>
				<?php } ?>
			</div>
			<?php } ?>
			...
			<table class="rsf_files table table-striped">
				<thead>
					<tr>
						...
						<?php if ($this->config->list_show_date) { ?><th width="12%"><?php echo JHtml::_('grid.sort', 'COM_RSFILES_FILE_DATE', 'date', $this->listDirn, $this->listOrder); ?></th><?php } ?>
						<th width="10%">&nbsp;</th>
					</tr>
				</thead>
				<tbody>
					...
					<tr class="row<?php echo $i % 2; ?>">
						...
						<td>
							...
							<?php if ($canDelete) { ?>
							<a href="<?php echo JRoute::_('index.php?option=com_rsfiles&task=rsfiles.delete&path='.rsfilesHelper::encode($item->fullpath).$this->itemid); ?>" class="<?php echo rsfilesHelper::tooltipClass(); ?>" title="<?php echo rsfilesHelper::tooltipText(JText::_('COM_RSFILES_NAVBAR_DELETE')); ?>" onclick="if (!confirm('<?php echo JText::_('COM_RSFILES_DELETE_FILE_MESSAGE',true); ?>')) return false;">
							...
							<?php } else if ($item->type == 'folder') { ?>
								...
								<?php if ($canDelete) { ?>
								<a href="<?php echo JRoute::_('index.php?option=com_rsfiles&task=rsfiles.delete&folder='.rsfilesHelper::encode($item->fullpath).$this->itemid); ?>" class="<?php echo rsfilesHelper::tooltipClass(); ?>" title="<?php echo rsfilesHelper::tooltipText(JText::_('COM_RSFILES_NAVBAR_DELETE')); ?>" onclick="if (!confirm('<?php echo JText::_('COM_RSFILES_DELETE_MESSAGE',true); ?>')) return false;">
									<i class="fa fa-trash fa-fw"></i>
								</a>
								<?php } ?>
